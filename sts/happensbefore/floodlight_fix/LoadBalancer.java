...
public class LoadBalancer implements IFloodlightModule,
    ...
    //(RM)
    // Enable and disable sending and handling of barrier request/replys
    private boolean enable_barrier = true;

    // (RM) random for xid generation
    Random rand = new Random();

    //(RM) Class to store packet information (RM)
    private ArrayList<PendingPackets> pending = new ArrayList<PendingPackets>();

    private class PacketInformation{
        // define variables for packet information
        IPacket packet;
        IOFSwitch sw;
        int bufferId;
        short inPort;
        ...
        void sendPacket(){
            pushPacket(packet, sw, bufferId, inPort, outPort, cntx, flush);
        }

    }

    private class PendingPackets{
        // indicate if the route is ready
        boolean route_ready = false;
        // List to store xids
        HashSet<Integer> xids = new HashSet<Integer>();
        // List to store packets
        ArrayList<PacketInformation> packets = new ArrayList<PacketInformation>();

        // variables for flow identification:
        int destIpAddress;
        int returnIpAddress;
        int srcIpAddress;
        int proto;

        void setIdentifiers(int dst, int src, int ret, int p){
            destIpAddress = dst;
            srcIpAddress = src;
            returnIpAddress = ret;
            proto = p;
        ...
        void addPacket(PacketInformation pkt){
            packets.add(pkt);
        }

        void sendPacket(){
            for(PacketInformation pkt : packets){
                pkt.sendPacket();
            }
        }
    }
    ...
    public net.floodlightcontroller.core.IListener.Command
        ...
        switch (msg.getType()) {
            ...
            // (RM) Add barrier reply handling
            case BARRIER_REPLY:
                if(!enable_barrier){
                   return Command.STOP;
                }
                log.debug("Received Packet: {}", msg.getType());
                PendingPackets pendPkt = null;
                for(PendingPackets pp : pending){
                    if (pp.xids.contains(msg.getXid())){
                        pp.xids.remove(msg.getXid());
                        pendPkt = pp;
                        log.debug("Removed xid {}.", msg.getXid());
                        break;
                    }
                }
                if (pendPkt == null){
                    log.debug("Did not find xid {}", msg.getXid());
                    // return Command.CONTINUE produces error...
                    return Command.STOP;
                }
                if(pendPkt.xids.isEmpty()){
                    pendPkt.sendPacket();
                    pendPkt.route_ready = true;
                    //pending.remove(pendPkt);
                    log.debug("Xids empty. Send packets, remove entry.");
                }
    ...
    private net.floodlightcontroller.core.IListener.Command
        ...
        if (eth.isBroadcast() || eth.isMulticast()) {
            ...
            if (pkt instanceof IPv4) {
                ...
                // (RM) Check if we wait for barrier replys for a corresponding rule
                ...
                int proto = ip_pkt.getProtocol();

                if(enable_barrier){
                    for (PendingPackets pp : pending){
                        if(((pp.destIpAddress == destIpAddress &&
                           pp.srcIpAddress == srcIpAddress) ||
                           (pp.returnIpAddress == srcIpAddress &&         // Todo: Change destIP to virtual IP
                           pp.srcIpAddress == destIpAddress)) &&
                           pp.proto == proto){

                                log.debug("found matching pending packet");
                                log.debug("Pkt: {},{},{}", new Object[] {destIpAddress, srcIpAddress,proto});
                                log.debug("pp: {},{},{}", new Object[] {pp.destIpAddress, pp.srcIpAddress,pp.proto});
                                // send the packet back to the switch if the route is ready, else append it to the packet list
                                if(pp.route_ready){
                                    // (RM) output packet
                                    pushPacket(pkt, sw, pi.getBufferId(), pi.getInPort(), OFPort.OFPP_TABLE.getValue(), cntx, true);
                                }else{
                                    PacketInformation pktInf = new PacketInformation(pkt, sw, pi.getBufferId(), pi.getInPort(), OFPort.OFPP_TABLE.getValue(), cntx, true);
                                    pp.addPacket(pktInf);
                                }
                           }
                           return Command.STOP;
                ...
                if (vipIpToId.containsKey(destIpAddress)){
                    ...
                    // (RM)
                    PendingPackets pendPkt = new PendingPackets();
                    pushBidirectionalVipRoutes(sw, pi, cntx, client, member, pendPkt);

                    // packet out based on table rule
                    if(enable_barrier){

                        if(pendPkt.xids.isEmpty()){
                            // we dont have to wait for replys -> send packets
                            log.debug("Direct packet output, no need to wait.");
                            pushPacket(pkt, sw, pi.getBufferId(), pi.getInPort(), OFPort.OFPP_TABLE.getValue(), cntx, true);
                        }else{
                            pendPkt.setIdentifiers(destIpAddress, srcIpAddress, member.address, proto);
                            PacketInformation pktInf = new PacketInformation(pkt, sw, pi.getBufferId(), pi.getInPort(), OFPort.OFPP_TABLE.getValue(), cntx, true);
                            pendPkt.addPacket(pktInf);
                            pending.add(pendPkt);
                            return Command.STOP;
                        }
                    }else{
     ...
     */
    protected void pushBidirectionalVipRoutes(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx, IPClient client, LBMember member, PendingPackets pendPkt) {
        ...
        while ((iSrcDaps < srcDaps.length) && (iDstDaps < dstDaps.length)) {
            ...
            if (srcVsDest == 0) {
                if (!srcDap.equals(dstDap) && 
                    ...
                    if (routeIn != null) {
                        pushStaticVipRoute(true, routeIn, client, member, sw.getId(), pendPkt);
                    }
                    
                    if (routeOut != null) {
                        pushStaticVipRoute(false, routeOut, client, member, sw.getId(), pendPkt);
                    }
     ...
     */
    public void pushStaticVipRoute(boolean inBound, Route route, IPClient client, LBMember member, long pinSwitch, PendingPackets pendPkt) {
        ...
        if (path.size()>0) {
           for (int i = 0; i < path.size(); i+=2) {
               ...
               // Send synchronisation barrier only the second time, (RM)
               if(enable_barrier){
                   if (!inBound) {
                     int xid = sendBarrier(sw);
                     pendPkt.xids.add(xid);
                     log.debug("Added xid {} to xids.", xid);
                   }
               }
           }
        }
    ...
    // Send Barrier Request, Edited (RM)
    private int sendBarrier(long dpid) {
        log.debug("Entered sendBarrier for SW: {}",dpid);

        // get IOFSwitch from dpid
        IOFSwitch sw = floodlightProvider.getSwitch(dpid);

        // Create new barrier request, send it to switch and create Listenable Future
        OFMessage barrierMsg = floodlightProvider.getOFMessageFactory().getMessage(OFType.BARRIER_REQUEST);

        // Set xid
        int xid = rand.nextInt(Integer.MAX_VALUE);
        barrierMsg.setXid(xid);

        try {
            sw.write(barrierMsg, null);
            log.debug("Send Barrier to sw: {}",dpid);
        } catch (IOException e) {
            log.debug("Error when sending barrier Reply to sw");
            e.printStackTrace();
        }
        sw.flush();
        return xid;
    ...
    public void startUp(FloodlightModuleContext context) {
        ...
        // (RM), add listener for BARRIER_REPLY
